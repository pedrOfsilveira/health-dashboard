<script>
  ...
  function getDifficultyColor(difficulty) {
    switch (difficulty) {
      case 'easy': return 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800';
      case 'medium': return 'bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-800';
      case 'hard': return 'bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 border-red-200 dark:border-red-800';
      case 'legendary': return 'bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 border-purple-200 dark:border-purple-800';
...
<div class="w-full max-w-[500px] mx-auto px-4 pb-10">
  ...
  <!-- Page Header -->
  <div class="mb-8">
    <h1 class="text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight">⚡ Desafios</h1>
    <p class="text-xs text-slate-500 dark:text-slate-400 font-medium mt-1">Complete desafios, ganhe XP e compita com amigos!</p>
  </div>
  ...
  <div class="flex gap-2 mb-6">
    <button
      ...
      class="flex-1 py-2.5 rounded-2xl text-xs font-black uppercase tracking-wider transition-all
        {activeTab === 'active' ? 'bg-slate-900 dark:bg-slate-700 text-white shadow-lg' : 'bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700'}"
    >
    ...
    <button
      ...
      class="flex-1 py-2.5 rounded-2xl text-xs font-black uppercase tracking-wider transition-all
        {activeTab === 'catalog' ? 'bg-slate-900 dark:bg-slate-700 text-white shadow-lg' : 'bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700'}"
    >
      Catálogo
    </button>
    {#if completedChallenges.length > 0}
      <button
        ...
        class="flex-1 py-2.5 rounded-2xl text-xs font-black uppercase tracking-wider transition-all
          {activeTab === 'completed' ? 'bg-slate-900 dark:bg-slate-700 text-white shadow-lg' : 'bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700'}"
      >
        Feitos ✓
      </button>
    {/if}
  ...
  {:else if activeTab === 'active'}
    {#if activeChallenges.length === 0}
      <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-8 text-center">
        ...
        <p class="text-sm font-bold text-slate-400 dark:text-slate-500">Nenhum desafio ativo</p>
        <p class="text-xs text-slate-400 dark:text-slate-500 mt-1">Escolha um desafio no catálogo para começar!</p>
        <button
          onclick={() => activeTab = 'catalog'}
    ...
    {:else}
      <div class="space-y-4">
        {#each activeChallenges as participation (participation.id)}
          ...
          <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl shadow-sm p-5 hover:shadow-md transition-all" style="animation: slideUp 0.3s ease-out">
            <!-- Header -->
            <div class="flex items-center gap-3 mb-4">
              <span class="text-3xl">{challenge?.icon || '🏆'}</span>
              <div class="flex-1">
                <h3 class="font-black text-slate-800 dark:text-slate-100 text-sm">{challenge?.title}</h3>
                <p class="text-[10px] text-slate-400 dark:text-slate-500 font-medium mt-0.5">{challenge?.description}</p>
              </div>
              <div class="text-right">
                <p class="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase">{daysLeft}d</p>
                <p class="text-[8px] font-bold text-slate-400 dark:text-slate-500">restantes</p>
              </div>
            </div>

            <!-- Progress bar -->
            <div class="mb-3">
              <div class="flex justify-between text-[10px] font-bold text-slate-400 dark:text-slate-500 mb-1.5">
                <span>{progress} / {target}</span>
                <span class="text-emerald-600 dark:text-emerald-400">{pct}%</span>
              </div>
            ...
            <div class="flex items-center justify-between">
              <div class="flex gap-2">
                <span class="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-1 bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded-lg border border-amber-200 dark:border-amber-800">
                ...
                {#if challenge?.type === 'duo'}
                  <span class="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-1 bg-violet-50 dark:bg-violet-900/30 text-violet-700 dark:text-violet-400 rounded-lg border border-violet-200 dark:border-violet-800">
                    🤝 Duo
                  </span>
                {/if}
              </div>
              {#if pct >= 100}
                <span class="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase">✓ Completo!</span>
              {/if}
            </div>
          </div>
        {/each}
  ...
  {:else if activeTab === 'catalog'}
    ...
    <div class="flex gap-2 mb-4">
      <button
        ...
        class="px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all
          {filterType === 'all' ? 'bg-slate-900 dark:bg-slate-700 text-white' : 'bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700'}"
      >
        Todos
      </button>
      <button
        ...
        class="px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all
          {filterType === 'solo' ? 'bg-slate-900 dark:bg-slate-700 text-white' : 'bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700'}"
      >
        🧍 Solo
      </button>
      <button
        ...
        class="px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all
          {filterType === 'duo' ? 'bg-slate-900 dark:bg-slate-700 text-white' : 'bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700'}"
      >
        🤝 Duo
      </button>
    </div>
    ...
    <div class="space-y-3">
      {#each filteredCatalog as challenge (challenge.id)}
        ...
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-5 hover:border-slate-300 dark:hover:border-slate-600 transition-all">
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
              <div class="flex items-center gap-2 mb-1">
                <h3 class="font-black text-slate-800 dark:text-slate-100 text-sm">{challenge.title}</h3>
                {#if challenge.type === 'duo'}
                  <span class="px-1.5 py-0.5 bg-violet-50 dark:bg-violet-900/30 text-violet-600 dark:text-violet-400 rounded-md text-[8px] font-black uppercase">DUO</span>
                {/if}
              </div>
              <p class="text-xs text-slate-500 dark:text-slate-400 mb-3">{challenge.description}</p>

              <div class="flex gap-2 flex-wrap mb-3">
                <span class="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-1 bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded-lg border border-amber-200 dark:border-amber-800">
                  ⚡ {challenge.xp_reward} XP
                </span>
                <span class="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-1 bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg border border-slate-200 dark:border-slate-600">
